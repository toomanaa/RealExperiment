...
while 1:
    ...
    while True:
        cpuUtilization.append(psutil.cpu_percent(0.5))
        cpuUtilization.append(psutil.cpu_percent(0.5))
        print(cpuUtilization)
        #if(psutil.cpu_percent(0.5)<= min(cpuUtilization)):
            #print(psutil.cpu_percent(0.5))
            # Now we can establish connection with clien
        conn, addr = s.accept()
            #print("we will go with the thrid server utilization")
        
        #elif(psutil.cpu_percent(0.5)<= min(cpuUtilization)): 
            #print(psutil.cpu_percent(0.5))
            #conn, addr = s.accept()
            #print("we will go with the forth server utilization")
        
            
        #else:
            #print(psutil.cpu_percent(0.5))
            #conn, addr = s.accept()
            #print("we will go with the fifth server utilization")
